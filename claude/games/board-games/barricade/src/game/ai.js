...
/* ── AI FLAVOR (event log commentary) ────────────────────────
   Small personality-flavored lines logged alongside the AI's own
   moves/walls, so the log reads like an opponent with character
   instead of a flat transaction record. The .ev.ai-flavor CSS class
   already existed in styles.css but nothing ever produced this
   content — this is what was missing. Not every turn logs one — a
   per-difficulty chance keeps it from feeling spammy, and higher
   difficulties get more "personality" since their moves are more
   deliberate to begin with. */
const AI_FLAVOR = {
  0: { move: ["shuffles a piece forward.", "wanders toward the goal."],
       wall: ["drops a wall... somewhere."] },
  1: { move: ["nudges a piece ahead.", "edges closer to the target."],
       wall: ["tries a speculative wall."] },
  2: { move: ["advances with purpose.", "closes the gap."],
       wall: ["blocks a likely route."] },
  3: { move: ["threads through the middle.", "presses the advantage."],
       wall: ["cuts off an approach."] },
  4: { move: ["calculates the optimal path.", "tightens the net."],
       wall: ["seals off a key corridor."] },
  5: { move: ["executes a precise advance.", "closes in relentlessly."],
       wall: ["cuts off your route entirely."] }
};
const AI_FLAVOR_CHANCE = [0.2, 0.25, 0.3, 0.35, 0.4, 0.5];

function maybeLogAiFlavor(difficulty, actionType) {
  if (Math.random() > (AI_FLAVOR_CHANCE[difficulty] ?? 0.3)) return;
  const names = ['Beginner','Amateur','Regular','Pro','Master','Legend'];
  const pool = AI_FLAVOR[difficulty]?.[actionType] || [];
  if (!pool.length) return;
  const line = pool[Math.floor(Math.random() * pool.length)];
  // Small delay so it lands after the move/wall's own event-log entries
  // (those are already scheduled inside doMove/placeWallAt).
  setTimeout(() => { if (typeof logEv === 'function') logEv(`🤖 ${names[difficulty]} AI ${line}`, 'ai-flavor'); }, 700);
}
...
function aiTakeTurn() {
  ...
  setTimeout(() => {
    ...
    if (dec.action === 'move') {
      ...
      if (mv) { doMove(G.aiPlayer, dec.idx, dec.r, dec.c, mv); maybeLogAiFlavor(G.aiDifficulty, 'move'); }
    } else {
      G.actionMode = 'wall';
      placeWallAt(dec.type, dec.r, dec.c);
      maybeLogAiFlavor(G.aiDifficulty, 'wall');
    }
  }, delay);
}
