        ...
        if (!disconnectTimer) {
          ...
          disconnectTimer = setInterval(() => {
            ...
            if (disconnectCountdown <= 0) {
              ...
              triggeredForfeit(); // Opponent failed 20s timer window -> you win by forfeit
            }
          }, 1000);
        }
      } else {
...
function beginPlayOnline() {
  ...
  // Push this immediately rather than waiting for the first move —
  // otherwise the shared room row stays showing phase:'waiting' even
  // though both clients have already locally moved into 'playing'.
  syncOnlineState();
}
...
window.addEventListener('message', (event) => {
    ...
    if (event.data && event.data.type === 'STEENE_AUTH_SYNC') {
        const { session, user } = event.data;
        console.log("Game synced with STEENE platform user:", user);
        
        // If your game uses Supabase locally, you can set the session directly:
