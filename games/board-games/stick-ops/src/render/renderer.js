export class Renderer{
  ...
  drawWorld(c,h,cam,s,zoom=1){
    ...
    // decals
    for(const d of s.decals){c.globalAlpha=Math.min(1,d.life/2);c.fillStyle=this.config.colors.blood;c.beginPath();c.arc(d.x,d.y,d.size*(1.2-d.life/15),0,Math.PI*2);c.fill()}c.globalAlpha=1;
    c.restore();
  }
  ...
  drawFighter(c,f,isPlayer=false){
    ...
    c.save();c.translate(f.x,f.y);c.scale(flip*scale,scale);c.lineCap="round";c.lineJoin="round";const hurt=f.flash>0;const body=f.color;
    ...
    // legs
    const stride=walk*9*speed,cBoot="#0b0e12";c.strokeStyle=body;c.lineWidth=8;c.beginPath();c.moveTo(-7,29);c.lineTo(-15+stride,61);c.lineTo(-23+stride,84);c.stroke();c.beginPath();c.moveTo(7,29);c.lineTo(15-stride,61);c.lineTo(25-stride,84);c.stroke();c.fillStyle=cBoot;c.beginPath();c.roundRect(-31+stride,80,17,7,3);c.fill();c.beginPath();c.roundRect(17-stride,80,17,7,3);c.fill();
    // torso tactical vest
    c.fillStyle=hurt?"#fff":body;c.beginPath();c.moveTo(-17,-17);c.lineTo(17,-17);c.lineTo(19,30);c.lineTo(-19,30);c.closePath();c.fill();c.fillStyle="#0c1118";c.fillRect(-12,-8,24,27);c.strokeStyle="rgba(255,255,255,.18)";c.lineWidth=2;c.strokeRect(-12,-8,24,27);c.fillStyle="rgba(255,255,255,.12)";c.fillRect(-8,-2,6,10);c.fillRect(2,-2,6,10);
    // head + helmet
    ...
    c.strokeStyle=armColor;c.lineWidth=f.type==="heavy"?10:7;c.beginPath();c.moveTo(10,-5);c.lineTo(23,elbowY);c.lineTo(gunBaseX+Math.cos(aa)*16,gunBaseY+Math.sin(aa)*16);c.stroke();c.beginPath();c.moveTo(-9,-2);c.lineTo(-20,12+walk*2);c.lineTo(gunBaseX+Math.cos(aa)*8,gunBaseY+Math.sin(aa)*8);c.stroke();
    // hands
    ...
    if(f.type==="boss"){c.strokeStyle="rgba(255,62,86,.45)";c.lineWidth=3;c.beginPath();c.arc(0,-18,36,0,Math.PI*2);c.stroke();}
    c.restore();
    // health / armor above head
    if(f.enemy&&!dead){const cls=cfg.enemyClasses[f.type];const bw=f.type==="boss"?128:50;c.fillStyle="rgba(0,0,0,.62)";c.fillRect(f.x-bw/2,f.y-102-(scale-1)*20,bw,7);c.fillStyle=cls.color;c.fillRect(f.x-bw/2,f.y-102-(scale-1)*20,bw*Math.max(0,f.hp/f.maxHp),7);if(f.armor>0){c.fillStyle="#6b809b";c.fillRect(f.x-bw/2,f.y-111-(scale-1)*20,bw*Math.max(0,f.armor/f.maxArmor),4)}if(f.type==="boss"){c.fillStyle="#f3f5f8";c.font="700 9px Inter,system-ui";c.textAlign="center";c.fillText("WARDEN",f.x,f.y-116)} }
  }
  ...
  draw(s,input){
    ...
    this.drawBackground(c,w,h,cam);this.drawWorld(c,h,cam,s,this.camera.zoom);
