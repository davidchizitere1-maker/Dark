export class AISystem{
  ...
  update(e,p,s,dt){
    if(e.dead)return;const cls=this.config.enemyClasses[e.type]||this.config.enemyClasses.grunt;const dx=p.x-e.x,dy=p.y-e.y,d=Math.hypot(dx,dy)||1;e.facing=dx>=0?1:-1;e.aiTimer+=dt;e.walkCycle+=Math.abs(e.vx)*dt*.015;e.dashCooldown=Math.max(0,e.dashCooldown-dt);
  ...
  updateBoss(e,p,s,dt){
    ...
    if(e.phase>=2&&e.dashCooldown<=0){e.vx+=Math.sign(p.x-e.x)*720;e.dashCooldown=e.phase===3?1.8:2.8;e.invulnerable=.2}
